...
(() => {
  ...
  async function api(path, data, authenticated=true) {
    ...
    try {
      ...
      if(!response.ok){const e=new Error(result.error||'Không thể kết nối phòng.');e.status=response.status;throw e;}
      return result;
  ...
  function renderRoom() {
    ...
    if(state.phase==='lobby') {
      ...
      if(host)body+=`<section class="live-class-report"><div class="live-section-heading"><h2>Kết quả tổng hợp của lớp</h2>${btn('download','Tải kết quả JSON','quiet')}</div><p class="live-small">Thống kê ${report.scored} câu đã chốt. Điểm không phụ thuộc tốc độ; câu bỏ lỡ nhận 0 điểm.</p>${report.breakdown.map(q=>`<details><summary>Câu ${q.index+1} · ${escape(q.topic)} — ${q.counts[q.correct]}/${q.total} trả lời đúng</summary><p>${escape(q.question)}</p><div class="live-distribution">${q.options.map((a,i)=>`<div><span>${'ABCD'[i]}${i===q.correct?' ✓':''}</span><p>${escape(a)}</p><b>${q.counts[i]} lượt</b></div>`).join('')}</div><p>Bỏ lỡ: ${q.skipped} · ${escape(q.difficulty)} · Slide ${q.source.slide} · ${escape(q.source.transcript)}</p></details>`).join('')}</section>`;
    }
    body+=hostButtons();
