...
next([sample('c3')]); await generateQuiz(true);
...
assert(request.exclude_concept_ids.includes('c1')&&request.exclude_concept_ids.includes('c2'),'Exclude used concepts');
assert(questions[0].status==='approved'&&questions[2].status==='pending','Keep approvals during refill');
const previous=JSON.stringify(questions); setFail(true); await generateQuiz();
assert(JSON.stringify(questions)===previous&&!isGenerating,'Failure preserves quiz and clears loading');
